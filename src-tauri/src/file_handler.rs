...
pub fn preprocess_data(input_dir: &Path, output_dir: &Path, filters: &[DimensionFilter]) -> io::Result<()> {
    ...
    if let Some(dir_name) = input_dir.components().last() {
        let output_path = output_dir.join(dir_name.as_os_str());

        for (dimensions, files) in dimension_map {
            if filters.iter().all(|f| f(&dimensions, &files)) {
                let output_subdir = output_path.join(format!("{}x{}", dimensions.0, dimensions.1));
        ...
        return Err(io::Error::new(io::ErrorKind::Other, "Input directory path is not valid"));
    }
